...
import { AGENT_VOICES, AGENT_PERSONALITIES, AGENT_LANGUAGES, INTERRUPT_SENSITIVITIES } from '@/constants';
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'configure' ? (
            ...
            <Card>
              ...
              <form onSubmit={handleSubmit(onSave)} className="space-y-5">
                ...
                <Textarea
                  label="System Prompt"
                  rows={8}
                  placeholder="You are a professional receptionist..."
          ...
          >
            <Card>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.2)' }}>
                ...
                <p className="text-[13px] max-w-md mb-6" style={{ color: '#4b6070' }}>
                  A business profile must be configured before testing agents. This links the agent to the correct business data and services.
                </p>
                <a
                  href="/dashboard/settings"
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-[13px] font-semibold text-white transition-all"
                  style={{ background: '#fbbf24', color: '#080e10', boxShadow: '0 4px 12px rgba(251,191,36,0.3)' }}
                >
          ...
          <AgentTestTab
            key="test"
            agent={agent}
            businessId={business?.id || agent?.business_id || ''}
          />
        )}
      </AnimatePresence>
    </div>
...
// ─── Test Tab Component (wrapped with AgentProvider) ─────────────────────────
function AgentTestTab({ agent, businessId }: { agent: Agent; businessId: string }) {
  const toast = useToast();
  ...
  const handleConnect = async () => {
    if (!businessId) { toast.error('No business configured'); return; }
    setCallDuration(0);
    await connect();
  };
